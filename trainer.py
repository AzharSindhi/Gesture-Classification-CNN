...
class Trainer:
    ...
    def train_step(self, x, linear_input, y):
        # perform following steps:
        # -reset the gradients. By default, PyTorch accumulates (sums up) gradients when backward() is called. This behavior is not required here, so you need to ensure that all the gradients are zero before calling the backward.
        self._optim.zero_grad()
        # -propagate through the network
        out = self._model(x, linear_input)
        # -calculate the loss
    ...
    def val_test_step(self, x, linear_input, y):

        # predict
        # propagate through the network and calculate the loss and predictions
        pred = self._model(x, linear_input)
    ...
    def train_epoch(self):
        ...
        for data in self._train_dl:
            ...
            loss, pred = self.train_step(x, linear_input, y)
            # pred = F.log_softmax(pred, dim=-1)
            if self.num_classes == 1:
                pred = torch.sigmoid(pred)
                yt = y.detach().cpu().numpy().flatten()#.argmax(axis=1).flatten()
                yp = pred.detach().cpu().numpy().round().flatten()#argmax(axis=1).flatten()
            else:
                pred = F.log_softmax(pred, dim=-1)
        ...
        self.scheduler.step()
        fscore = f1_score(ytrue, ypred, average=None)
    ...
    def val_test(self):
        ...
        with t.no_grad():
            ...
            for data in self._val_test_dl:
                ...
                loss, pred = self.val_test_step(x, linear_input, y)
                # pred = F.log_softmax(pred, dim=-1)
                if self.num_classes == 1:
                    pred = torch.sigmoid(pred)
                    yt = y.detach().cpu().numpy().round().flatten()
                    yp = pred.detach().cpu().numpy().round().flatten()
                else:
                    pred = F.log_softmax(pred, dim=-1)
                    yt = y.detach().cpu().numpy().argmax(axis=1).flatten()
                    yp = pred.detach().cpu().numpy().argmax(axis=1).flatten()
            ...
            return average_test_loss, f1score
    ...
    def fit(self, epochs=-1):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for i, class_name in enumerate(self.class_names):
                mlflow.log_metric(
                    "train_" + class_name + "_fscore", fscore_train[i], step=epoch
                )
                mlflow.log_metric(
                    "val_" + class_name + "_fscore", fscore_val[i], step=epoch
                )
